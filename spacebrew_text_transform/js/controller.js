...
Controller.app.prototype = {
	...
	debug: true,					// flag that identifies if debug messages should be output
	...
	map: function(_inletName, _incomingMsg) {
		...
		for(var i in this.model.transforms.maps) {
			...
			if (this.model.transforms.maps[i]["inlet"].indexOf(_inletName) == 0 || 
				...
				if ((this.model.transforms.maps[i]["incoming"].indexOf(_incomingMsg) == 0) && 
					(this.model.transforms.maps[i]["incoming"].length == _incomingMsg.length)) {
					this.sendMsg(i);
