...
var onOpen = function () {
  ...
  var newString = "Connection established with server at '" + model.server + "'\n\n";
...
var onClose = function () {
  ...
  var newString = "Connection attempt failed or spacebrew server no longer available at '" + model.server + "'\n\n" +
  ...
  if (!window.getQueryString('server')) {
    ...
    newString = newString + "This is how you can specify the server address in the webapp url (" +
                            ...
                            model.href + conn + "server=server_address&"             
  }
  $("#statusMsgDiv p").html(htmlForTextWithEmbeddedNewlines(newString));
}